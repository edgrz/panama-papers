...
occurrences
```
Filtering by subtypes which has more than 30 occurrences,we ended up with 6 kind of relationships. However, *beneficional owener of* and *beneficiary of* can be merged as one, as they refer to same concept:
...
```{r}
#edges_parsed$TYPE <- gsub('\\s+', '_', edges_parsed$link)
#new_occurrencies <- table(edges_parsed$TYPE)
#new_occurrencies
edges_parsed$TYPE <- edges_parsed$link
```
CSV export:
```{r}
write.csv(edges_parsed, file = "../../data/edges_parsed.csv",row.names = FALSE)
```
